...
- [x] **AC-6**: 毎日0時（JST）の定期実行でビルド・デプロイが走る（`schedule` トリガー）
...
- [x] **AC-8**: 非公開リポジトリの `main` に新規ページを push した翌0時すぎ、そのページが公開される
...
- [x] カットオーバー（1ブランチをまとめてマージ）→ 手動デプロイ・定期デプロイの動作確認（マージ・push デプロイ・手動デプロイは完了。定期デプロイの確認待ち）
