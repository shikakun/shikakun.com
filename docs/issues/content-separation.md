...
- [ ] **AC-6**: 毎日0時（JST）の定期実行でビルド・デプロイが走る（`schedule` トリガー） ※実装済み。次回の定期実行（JST 6/25 0時すぎ）で検証予定
- [x] **AC-7**: `main` への code push、および手動実行（`workflow_dispatch`）でもビルド・デプロイが走る（push: run 28060921170 / workflow_dispatch: run 28061401830 で検証済み）
- [ ] **AC-8**: 非公開リポジトリの `main` に新規ページを push した翌0時すぎ、そのページが公開される ※実装済み。確認用ページを非公開 main へ push し、次回の定期実行（JST 6/25 0時すぎ）で検証予定
...
- [ ] カットオーバー（1ブランチをまとめてマージ）→ 手動デプロイ・定期デプロイの動作確認（マージ・push デプロイ・手動デプロイは完了。定期デプロイの確認待ち）
