...
describe('formatDate', () => {
  it('年月日時刻の場合、英語表記で `{時刻} · {月} {日}, {年}` の形式で返す', () => {
    expect(formatDate('2026-04-23 15:06')).toBe('3:06 PM · Apr 23, 2026');
  });
  ...
  it('想定していない文字列が含まれる場合、空文字を返す', () => {
    expect(formatDate('2026-04-xx')).toBe('');
  });

  it('時刻が想定していない形式の場合、空文字を返す', () => {
    expect(formatDate('2026-04-23 9:06')).toBe('');
  });
});
